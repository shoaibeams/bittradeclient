...
export interface ValidationAttribute {
  ...
  GetErrorMessage(displayName: string): any;
}
...
export class RequiredValidator implements ValidationAttribute {
  ...
  GetErrorMessage(displayName: string): string {
    this.errorMessage = StaticHelper.formatString(
...
export class RequiredIfValidator implements ValidationAttribute {
  constructor(
    dependentAttrDisplayName: string,
    dependentAttrTargetValue: any,
    dependentAtrrValue: any,
    errorMsgFormat: string
  ) {
    this.errorMessageFormat = errorMsgFormat;
  ...
  IsValid(value: any): boolean {
    ...
    if (this.dependentAttributeTargetValue != null) {
      if (typeof this.dependentAttributeTargetValue != "undefined") {
        if (
          this.dependentAttributeValue != this.dependentAttributeTargetValue
        ) {
          return true;
        }
      }
  ...
  GetErrorMessage(displayName: string): string {
    this.errorMessage = StaticHelper.formatString(
      this.errorMessageFormat,
      displayName,
      this.dependentAttributeDisplayName
    );
    return this.errorMessage;
  }
  errorMessage: string;
...
export class RegexValidator implements ValidationAttribute {
  constructor(regexString: any, errorMsgFormat: string, ...args) {
    ...
    this.regex = new RegExp(
      StaticHelper.formatString(regexString, ...this.argss)
    );
    this.innerAttribute = new RegexpValidator(
      this.errorMessageFormat,
      this.regex,
      ...this.argss
    );
  ...
  GetErrorMessage(displayName: string): string {
    if (!Array.isArray(this.errorMessageFormat)) {
      this.errorMessageFormat = [this.errorMessageFormat];
    }
    this.errorMessage = this.errorMessageFormat.map(m => {
      return StaticHelper.formatString(m, displayName, ...this.argss);
    });
    return this.errorMessage;
  }
  errorMessage: any;
