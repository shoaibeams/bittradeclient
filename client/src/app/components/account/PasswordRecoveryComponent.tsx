...
export class PasswordRecoveryComponent extends BaseComponent {
    ...
    return (
      <>
        <div className="gx-login-container" style={{ overflow: 'auto', paddingBottom: '120px' }}>
          <div className="gx-login-content" style={{ maxWidth: '500px', margin: "unset" }}>
            ...
            <div className="gx-login-header">
              <h1 className="gx-login-title">{this.lang.PassowrdRecovery}</h1>
            </div>
            {
              this.state.tokenVerified ?
                <div>
                  <div className="gx-flex-row">
                    <span className="gx-mb-2 gx-mr-3">{this.lang.SetNewPassword}</span>
                  </div>
                  <Form layout="vertical" onSubmit={this.onSubmit} className="gx-form-row0">
                    {
                  ...
                  </Form>
                  {
                    this.state.redirectToLogin ?
                      <div className="gx-mb-2">
                        <p>{this.lang.RedirectingYouToLogin + " " + this.state.threeDots}</p>
                      </div>
                      : null
                  }
                </div>
                  ...
                  <div className="gx-flex-row gx-mb-2">
                    <span>{
                      this.state.invalidToken ?
                        <>
                          {StaticHelper.formatString(this.lang.InvalidFormat, this.lang.Token) + ". " + 
                          this.lang.Try + " "}
  ...
  verifyPassowrdRecoveryToken = () => {
    this.http.post<mdCallResponse>(this.constants.EndPoints.PostAccountPasswordRecoveryToken,
  ...
  onSubmit = (e) => {
    ...
    this.http.post<mdCallResponse>(this.constants.EndPoints.PostAccountUpdatePassword, {
      ...formData,
      key: this.parsedLocation.key
    }).then((res: mdCallResponse) => {
      this.log.debug(res);
