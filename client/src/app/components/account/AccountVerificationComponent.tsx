...
export default class AccountVerificationComponent extends BaseComponent {
  ...
  render() {
    ...
    if (this.state.verified == 1 && this.state.progress == 100) {
      ...
      if (this.state.verified == 2 && this.state.progress == 100) {
        progressStatus["status"] = "exception";
      }
    return (
      <>
        <div className="gx-text-center">
          <img className="gx-mt-5 gx-mb-2"
            src="/assets/images/footer-logo.png"
            width={180}
            height={88}></img>
        </div>
        <div className="gx-login-container" style={{ overflow: 'auto', paddingBottom: '120px', height: 'unset' }}>
          <div className="gx-login-content" style={{ maxWidth: '500px' }}>
            <div className="gx-login-header gx-text-center">
              <h1 className="gx-login-title">{this.lang.AccountVerification}</h1>
            </div>
            <div className="gx-mb-3 gx-text-center">
              {
                this.animatedCSSDiv(
                  this.state.verified == 0 ? this.faicon("envelope-open-text", "10x") :
                    <Progress {...progressStatus} type="circle" percent={this.state.progress} width={150} />
                  , this.state.animValues.mail_img)
              }
            </div>
            <div style={{ alignContent: 'center' }}>
              <h2 className="gx-text-primary">{this.state.verificationResponse +
                (this.state.verified != 2 ? this.state.threeDots : '')}</h2>
            </div>
          </div>
        </div>
      </>
  ...
  init() {
    this.state = {
      ...
      animValues: {
        tick_img: new mdAnimControl(this.getTransisition(Transitions.pulse, TransitionState.Running),
          'tick_img', true),
        mail_img: new mdAnimControl(this.getTransisition(Transitions.pulse, TransitionState.Running),
          'mail_img', true),
      }
    }
