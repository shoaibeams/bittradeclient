...
export default class TradeBriefHistoryComponent extends BaseComponent {
  ...
  render() {
    ...
    return (
      <Widget styleName="gx-order-history"
        title={
          <h2 className="h4 gx-text-capitalize gx-mb-0">{this.lang.BriefHistory}</h2>
        } extra={
        ...
        >
          {
            this.markets ?
              this.markets.map((m, i) => {
                return (
                  <TabPane tab={
                    <div className="gx-text-center" onClick={() => {
  ...
  columns = [
    {
      ...
      width: 110,
      render: (data, p) => {
        return <div className="">{this.getCP(p.id).fc_name + "/" + this.getCP(p.id).tc_name}</div>;
      }
    },
    {
    ...
    },
    {
      ...
      render: (data, p) => {
        ...
        return <div className={p.change >= 0 ? 'green-txt' : 'red-txt'}>{p.change + "%"}</div>
      }
    },
    {
    ...
    },
    {
      ...
      width: 237,
      render: (data, p) => {
        return this.animatedCSSDiv(`${p.volume} ${this.getCP(p.id).fc_name}`, p.volumeAnim, (e) => {
  ...
  componentWillReceiveProps(newProps: mdProps) {
    this.initShorts(newProps);
    this.init();
  }
  ...
  changePairHistoryTab(name: string) {
    ...
    let brfHistory = this.g.briefHistory.filter(m => {
