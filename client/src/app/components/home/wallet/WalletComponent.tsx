...
export default class WalletComponent extends BaseComponent {
  render() {
    ...
    let getWalletBox = (
      pair: any,
      last: mdFormControl,
      volume: mdFormControl,
      bgColor: string,
      barsFillColor: string
    ) => {
      let data = pair.data;
    ...
    return (
      <>
        <Row className="walletwrap">
          {getWalletBox(
            this.state.firstPair,
            this.state.animValues.firstLast,
            this.state.animValues.firstVolume,
            "primary",
            "#092453"
          )}
          {getWalletBox(
            this.state.secondPair,
            this.state.animValues.secondLast,
            this.state.animValues.secondVolume,
            "orange",
            "#C87000"
          )}
          {getWalletBox(
            this.state.thirdPair,
            this.state.animValues.thirdLast,
            this.state.animValues.thirdVolume,
            "teal",
            "#158765"
          )}
          {getWalletBox(
            this.state.fourthPair,
            this.state.animValues.fourthLast,
            this.state.animValues.fourthVolume,
            "pink",
            "#BB1258"
          )}
        </Row>
    ...
    if (pair.id) {
      ...
      this.http
        .post<mdCallResponse>(this.constants.EndPoints.PostGetTrades, {
          ...
          last: pair.last,
          recordsPerPage: this.tradesPerCard
        })
        .then((res: mdCallResponse) => {
          if (res.isSuccess) {
            ...
            let trades = pair.trades as any[];
            if (res.extras.length > 0) {
              trades = [...trades, ...res.extras];
            }
            trades = trades.map(m => {
              m["key"] = m.id;
              m["name"] = m.last;
            ...
            if (trades.length > this.tradesPerCard) {
              trades.splice(0, Math.abs(this.tradesPerCard - trades.length));
            }
  ...
  loadPairs = () => {
    ...
    if (histr) {
      ...
      this.updateState(newState, () => {
        if (this.recentTradesWorkers.length < 1) {
  ...
  setAnimValuesForPair() {
    let animValues = this.state.animValues;
    if (this.old != null) {
      ...
      if (old) {
        ...
        if (old.last > this.state.firstPair.last) {
          animValues.firstLast = new mdFormControl(
        ...
        if (old.volume > this.state.firstPair.volume) {
          animValues.firstVolume = new mdFormControl(
            this.runTransition(Transitions.lesser),
            "firstVolume"
          );
        ...
        if (
          animValues.firstLast.value != "origional" ||
          animValues.firstVolume.value != "origional"
        ) {
      ...
      if (old) {
        ...
        if (old.last > this.state.secondPair.last) {
          animValues.secondLast = new mdFormControl(
            this.runTransition(Transitions.lesser),
            "secondLast"
          );
        ...
        if (old.volume > this.state.secondPair.volume) {
          animValues.secondVolume = new mdFormControl(
            this.runTransition(Transitions.lesser),
            "secondVolume"
          );
        ...
        if (
          animValues.secondLast.value != "origional" ||
          animValues.secondVolume.value != "origional"
        ) {
      ...
      if (old) {
        ...
        if (old.last > this.state.thirdPair.last) {
          animValues.thirdLast = new mdFormControl(
            this.runTransition(Transitions.lesser),
            "thirdLast"
          );
        ...
        if (old.volume > this.state.thirdPair.volume) {
          animValues.thirdVolume = new mdFormControl(
            this.runTransition(Transitions.lesser),
            "thirdVolume"
          );
        ...
        if (
          animValues.thirdLast.value != "origional" ||
          animValues.thirdVolume.value != "origional"
        ) {
      ...
      if (old) {
        ...
        if (old.last > this.state.fourthPair.last) {
          animValues.fourthLast = new mdFormControl(
            this.runTransition(Transitions.lesser),
            "fourthLast"
          );
        } else if (old.last < this.state.fourthPair.last) {
          animValues.fourthLast.value = new mdFormControl(
            this.runTransition(Transitions.greater),
            "fourthLast"
          );
        } else {
        ...
        if (old.volume > this.state.fourthPair.volume) {
          animValues.fourthVolume = new mdFormControl(
            this.runTransition(Transitions.lesser),
            "fourthVolume"
          );
        } else if (old.volume < this.state.fourthPair.volume) {
          animValues.fourthVolume.value = new mdFormControl(
            this.runTransition(Transitions.greater),
            "fourthVolume"
          );
        } else {
        ...
        if (
          animValues.fourthLast.value != "origional" ||
          animValues.fourthVolume.value != "origional"
        ) {
