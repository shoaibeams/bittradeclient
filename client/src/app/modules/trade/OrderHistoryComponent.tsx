...
export default class OrderHistoryComponent extends BaseComponent {
  ...
  render() {
    ...
    if (this.state.orders) {
      pendingOrders = this.state.orders.filter(m => m.record_status == OrderRecordStatuses.partially_completed ||
        m.record_status == OrderRecordStatuses[OrderRecordStatuses.partially_completed] ||
        m.record_status == OrderRecordStatuses.open ||
        m.record_status == OrderRecordStatuses[OrderRecordStatuses.open]);
      previousHistory = this.state.orders.filter(m => m.record_status == OrderRecordStatuses.cancelled ||
        m.record_status == OrderRecordStatuses[OrderRecordStatuses.cancelled] ||
    ...
    return (
      <Widget styleName="gx-order-history"
        title={
          <h2 className="h4 gx-text-capitalize gx-mb-0">{this.lang.OrderHistory}</h2>
        } extra={
          <p className="gx-text-primary gx-mb-0 gx-pointer">{this.lang.DetailedHistory}</p>
        }>
        <Tabs
          defaultActiveKey="1"
        ...
        >
          <TabPane tab={this.lang.PendingOrders} key="1">
            <div className="gx-table-responsive">
              <Table className="gx-table-no-bordered" columns={this.columns as any}
                dataSource={pendingOrders} pagination={false} bordered={false}
                size="small" 
                scroll={{x: 900, y: 300}}
                />
            </div>
          ...
          <TabPane tab={this.lang.PreviousHistory} key="2">
            <div className="gx-table-responsive">
              <Table className="gx-table-no-bordered" columns={this.columns as any}
                dataSource={previousHistory} pagination={false} bordered={false}
                size="small" scroll={{x: 900, y: 300}}/>
            </div>
          </TabPane>
        </Tabs>
