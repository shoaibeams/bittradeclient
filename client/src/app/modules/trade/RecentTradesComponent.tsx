...
export default class RecentTradesComponent extends BaseComponent {
  ...
  startSyncingRecentTraded = (timeout: number = this.constants.LoadBriefHistoryTimeout) => {
  ...
  columns = [
    {
      title: <>{this.lang.Time} <br />{"(" + this.lang.Local + ")"}</>,
      dataIndex: 'timestamp',
      render: (data) => {
    ...
    },
    {
      ...
      render: (text) => {
        ...
        return (
          <span className={`${text == OrderActions[OrderActions.buy] || text == OrderActions.buy ? 
          'gx-text-green' : 'gx-text-red'}`}>
            {value}
          </span>
        );
      },
