...
export default class DepositComponent extends BaseComponent {
                ...
                >
                  {
                    this.state.depositableCurrenciesSource.map((s, i) => {
                      return <Option key={i} value={s.value}>
                  ...
                  <table className="funding-wallet">
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td>{wallet ? this.state.wallet.balance.toFixed(cp.scale) + ' ' + cp.name : ''}</td>
                      </tr>
                      <tr>
                        <th>{this.lang.OnHold}</th>
                        <td>{wallet ? this.state.wallet.hold_balance.toFixed(cp.scale) + ' ' + cp.name : ''}</td>
                      </tr>
                      <tr>
                        <th>{this.lang.AvailableBalance}</th>
                        <td>{wallet ? this.state.wallet.available_balance.toFixed(cp.scale) + ' ' + cp.name : ''}</td>
                      </tr>
                    </tbody>
                  </table>
                }{
                    ...
                    <table className="table-responsive ">
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td>{sdm ? this.state.selectedCurrency.symbol + sdm.minimum_deposit : ''}</td>
                        </tr >
                        {
            ...
            this.antd.colmd12(
              this.animatedCSSDivWithAttr(
                <>
                  {
                    this.animatedCSSDivWithAttr(
                      <Card className="gx-card" title={this.lang.New + " " + this.lang.Deposit}>
                        ...
                        <Form onSubmit={this.submitDepositRequest} className="gx-signin-form gx-form-row0">
                          ...
                          <Row>
                            <Col xs={this.formItemLayout.labelCol.xs}
                              ...
                              md={this.formItemLayout.labelCol.md} style={{ paddingLeft: 0 }}>
                              <label style={{ margin: '0 8px 0 2px' }}>{this.lang.Reference}</label>
                            </Col>
                          ...
                          <Row>
                            <Col xs={this.formItemLayout.labelCol.xs}
                              sm={this.formItemLayout.labelCol.sm}
                              md={this.formItemLayout.labelCol.md} style={{ paddingLeft: 0 }}>
                              <label style={{ margin: '0 8px 0 2px' }}>{this.lang.DepositReceipt}</label>
                            </Col>
                            <Col {...this.formItemLayout.wrapperCol}>
                  ...
                  <Col span={8} offset={16}>
                    ...
                    >
                      ...
                      <i className="fa fa-plus"></i>&nbsp;<span>{this.lang.NewDepositRequest}</span>
                    </Button>
                  </Col>
                  <Card className="gx-card"
                    title={this.lang.DepositRequests}
                    extra={<p className="gx-text-primary gx-mb-0 gx-pointer">{this.lang.DetailedHistory}</p>}>
                    <Table className="gx-table-responsive"
        ...
        },
        {
          ...
          render: (value: any, row: mdDepositRequestHistory) => {
            ...
            if (value == DepositRequestRecordStatuses.pending) {
              ...
              if (value == DepositRequestRecordStatuses.rejected) {
                return <Tag color="red">{displayValue}</Tag>;
              }
              else
                if (value == DepositRequestRecordStatuses.success) {
  ...
  depositableCurrenciesOnChange = (value) => {
    ...
    let selectedCurrency = this.state.depositableCurrencies.filter(m => m.id == value)[0];
    this.updateState({
      ...
      this.http.post<mdCallResponse>(this.constants.EndPoints.PostDepositMethods,
        { currency_id: value }).then((data: mdCallResponse) => {
          this.depositMethods = [];
          this.feeSlabs = [];
          if (data.isSuccess) {
    ...
    this.http.post<mdCallResponse>(this.constants.EndPoints.PostDepositRequest, formData).then((res: mdCallResponse) => {
      ...
      if (res) {
        if (res.isSuccess) {
          ...
          this.successNotification(StaticHelper.formatString(this.lang.CreatedSuccessfullyFormat,
            this.lang.DepositRequest), this.lang.Success)
          this.updateState({
