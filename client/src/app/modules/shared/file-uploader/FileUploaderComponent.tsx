...
export default class FileUploaderComponent extends BaseComponent {
    ...
    return (
      <div className="file-uploader col-md-12">
        <input
          ...
          type="file"
          id={this.fileInputId}
          multiple={this.p.config.multiple}
          style={{ display: "none" }}
          accept={this.p.config.formatsAllowed.join(",")}
          ref={ref => (this.fileInput = ref)}
        />
        <label htmlFor={this.fileInputId} className="btn btn-primary">
          {this.p.config.text}
        </label>
        <button
        ...
        <label
          ...
          className="info-label text-danger"
        >
          {this.state.status}
        </label>
        <br />
        ...
        {this.state.filesToUpload
          ? this.state.filesToUpload.map((f, i) => {
              return (
                <div
                  key={i}
                  className="row nb-theme-default stylish-border stylish-row"
                >
                  <div className="col-md-7">
                    <label className="info-label">{f.fileName}</label>
                  </div>
                  <div className="col-md-5">
                    <Progress percent={f.progressBar.progress} />
                    {/* {
  ...
  setInfoText() {
    this.state = {
      ...
      infoText:
        "(" +
        this.p.config.formatsAllowed.join(", ") +
        ") " +
        this.lang.SizeLimit +
        " " +
        this.p.config.maxSize +
        " MB"
    };
  }
  ...
  fileSelectionChanged = event => {
    ...
    for (let i = 0; i < event.target.files.length; i++) {
      ...
      if (
        this.p.config.formatsAllowed.indexOf(
    ...
    return this.http
      .postFile(
        ...
        event => {
          ...
          if (
            Math.round(this.state.filesToUpload[i].progressBar.progress) !==
            newProgress
          ) {
      ...
      .then((res: mdCallResponse) => {
        ...
        this.updateState({
          ...
          status: res.isSuccess
            ? this.state.status
            : this.lang.SomethingWentWrongTryAgainLater
        });
      })
      .catch(error => {
        this.p.isBusyChange(false);
