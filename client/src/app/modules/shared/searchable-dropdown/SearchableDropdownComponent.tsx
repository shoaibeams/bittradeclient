...
export default class SearchableDropdownComponent extends BaseComponent {
    ...
    render() {
        ...
        return (
            <div className="dropdown dropdown-scroll sadd">
                                                ...
                                                <span>
                                                    {
                                                        this.p.settings.image ? (
                                                            <img width={this.p.settings.imageWidth} height={this.p.settings.imageHeight}
                                                                src={this.state.selectedValue.image} />) : (null)
                                                    }
                ...
                <ul className="dropdown-menu col-md-12" role="menu" aria-labelledby="dropdownMenu1">
                    {
                        this.p.settings.searchable ? (
                            <li role="presentation">
                                <div className="input-group input-group-sm search-control">
                                    ...
                                    {
                                        this.textFormControl(this.f.searchText, [this.handleFormControlInput, this.applySearch])
                                    }
                    ...
                    {
                        this.state.appliedSource ? this.state.appliedSource.map((s, i) => {
                            return (
                                <li key={i} role="presentation" onClick={(e) => { this.selectionChanged(s) }}>
                                    <a>
                                        {
                                            this.p.settings.image ? (
                                                <img width={this.p.settings.imageWidth} height={this.p.settings.imageHeight} src={s.image} />
                                            ) : (null)
                                        }
                                        &nbsp;{s.text}
                                    </a>
                ...
                <NBSpinnerComponent {...this.props} params={{ show: this.p.showSpinner }} />
            </div>
        );
    }
    ...
    init() {
        this.state = {
            form: {
                searchText: new mdFormControl('', 'searchText', this.p.settings.searchBoxPlaceholder),
            },
            selectedValue: null,
            appliedSource: null,
    ...
    recievedNewChanges() {
        this.state = {
            ...
            buttonText: this.state.selectedValue ? this.state.buttonText : this.p.settings.placeholder,
        }
        // if (changes.settings) {
    ...
    selectionChanged(value) {
        if (this.state.selectedValue != value) {
            this.p.onChange({ newValue: value, previousValue: this.state.selectedValue });
        }
        this.updateState({
            selectedValue: value
        }, () => {
    ...
    applySearch = (ev) => {
        ...
        if (st != '') {
            if (this.p.settings.searchCaseSensitive) {
                ...
                st = st.toUpperCase();
                newAppliedSource = this.p.source.filter(m => m.text.toUpperCase().indexOf(st) > -1);
            }
        }
        else {
