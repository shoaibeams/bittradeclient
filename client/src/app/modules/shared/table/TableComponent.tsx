...
export default class TableComponent extends BaseComponent {
  render() {
    ...
    return (
      <table
        {...this.p.settings.attr}
        className={this.p.settings.attr.className + " smart-table"}
      >
        <thead>
          <tr>
            {Object.keys(cols).map((m, i) => {
              return (
                <th key={i} style={cols[m].cellStyle}>
                  {cols[m].title}
                </th>
              );
            })}
          ...
          {this.p.data.map((d, i) => {
            return (
              <tr
                key={i}
                className={
                  this.p.settings.rowClassFunction
                    ? this.p.settings.rowClassFunction()
                    : ""
                }
              >
                {Object.keys(cols).map((c, j) => {
                  ...
                  return (
                    <td key={j} style={cols[c].cellStyle}>
                      {value}
                    </td>
                  );
                })}
